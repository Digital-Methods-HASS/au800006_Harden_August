...
DANmap <- leaflet() %>%   
  setView(11.13933, 56.03933 , zoom = 7) %>% 
  addTiles()

DANmap # To check the result.
...
# Most of the points are therem but some points are missing, yes! The reason for this is that some of the other students 
# forgot to type in their coordinates in the "Latitude" and "Longitude" coloumns.
...
# Solution
esri <- grep("^Esri", providers, value = TRUE) # Selecting backgrounds by grabbing their names.

for (provider in esri) {
  DANmap <- DANmap %>% addProviderTiles(provider, group = provider) # Selecting backgrounds from among provider tiles.
}

DANmap_layered <- DANmap %>%
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             popup = paste(places$Description, "<br>", places$Type)) %>% 
  addLayersControl(baseGroups = names(esri),
...
DANmap_layered # To check the result.
...
DANmap_clustered <- DANmap %>%
  ...
  addMarkers(lng = places$Longitude, 
             lat = places$Latitude,
             clusterOptions = markerClusterOptions()) %>%                  # Here I have added the function to cluster the points.
  addLayersControl(baseGroups = names(esri),
                   options = layersControlOptions(collapsed = FALSE)) %>% 
  addMiniMap(tiles = esri[[1]], toggleDisplay = TRUE,
             position = "bottomright") %>%
...
DANmap_clustered # To check the result.
...
# Too many individual points can make the browser lag. That's why the use of 
# clustering is much more efficient for the browser. Furthermore, clustering 
# makes the map more visible thus allowing us to actually see the map beneath 
# instead of icons overlapping and covering the entire map. By not using 
# clustering, we can actually see the right density of points thus allowing 
# us to see a pattern at a single glance. In addition, when points are 
# unclustered, we can hover and click/interact with a specific point of 
# interest immediately instead of the need to dig down through several points 
# in the map.
...
# Solution
gs4_deauth() # To de-authorize if it's a public sheet.

DANmap_descriptions <- DANmap_clustered %>%
  addTiles() %>% 
  addMarkers(lng = places$Longitude, 
                        lat = places$Latitude,
                        popup = paste(places$Description, "<br>", places$Type),
...
DANmap_descriptions # To check the result.
